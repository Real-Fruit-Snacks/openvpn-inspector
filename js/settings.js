...
(function () {
  ...
  function fire(key, val) {
    ...
    if (String(key).indexOf("ovpn-pet") === 0) window.dispatchEvent(new Event("tw:pet"));
  }
  ...
  panel.innerHTML =
    ...
    settingRow("THEME", '<span class="seg" data-pref="ovpn-theme">' +
      '<button data-val="dark">DARK</button><button data-val="light">LIGHT</button>' +
      '<button data-val="system">SYSTEM</button></span>') +
    settingRow("ACCENT", '<span class="seg accent-seg" data-pref="ovpn-accent">' +
    ...
    settingRow("MODE", '<span class="seg" data-petpref="ovpn-pet">' +
    ...
    settingRow("NAP", petToggle("ovpn-pet-nap")) +
    settingRow("FLEE", petToggle("ovpn-pet-flee")) +
    settingRow("READ", petToggle("ovpn-pet-read")) +
    settingRow("TRICKS", petToggle("ovpn-pet-tricks")) +
    settingRow("SPEECH", petToggle("ovpn-pet-speech")) +
    "</div></div>";
  ...
  function petDefaults(key) {
    return { "ovpn-pet": "float", "ovpn-pet-nap": "on", "ovpn-pet-flee": "on",
      "ovpn-pet-read": "on", "ovpn-pet-tricks": "on", "ovpn-pet-speech": "off" }[key];
  }
  ...
  function refreshSeg() {
    ...
    for (var i = 0; i < segs.length; i++) {
      ...
      var cur;
      if (key === "ovpn-theme") cur = TW.prefs.get("ovpn-theme", "system", ["dark", "light", "system"]);
      else cur = TW.prefs.get("ovpn-accent", "0", ["1", "2", "3", "4", "5"]);
  ...
  function refreshPet() {
    ...
    document.getElementById("pet-size").value = TW.prefs.get("ovpn-pet-size", "28");
    document.getElementById("pet-opacity").value = TW.prefs.get("ovpn-pet-opacity", "70");
  }
  ...
  panel.addEventListener("click", function (e) {
    ...
    if (key === "ovpn-theme") {
      if (val === "system") { TW.prefs.remove("ovpn-theme"); }
      else { TW.prefs.set("ovpn-theme", val); }
      applyTheme(val === "system" ? null : val);
      refreshSeg();
      return;
    }
    if (key === "ovpn-accent") {
      if (val === "0") { TW.prefs.remove("ovpn-accent"); }
      else { TW.prefs.set("ovpn-accent", val); }
